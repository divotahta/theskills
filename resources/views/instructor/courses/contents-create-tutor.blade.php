...
@section('content')
    <div>
                ...
                <div>
                    <div class="flex items-center space-x-2 text-sm text-gray-500 mb-2">
                        ...
                        <a href="{{ route('instructor.courses.contents.index', $course) }}"
                            class="hover:text-blue-600">Contents</a>
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                        </svg>
                        <span class="text-gray-900">Add Content</span>
                    </div>
                    <h1 class="text-3xl font-bold text-gray-900">Add Course Content</h1>
                    <p class="text-gray-600 mt-2">Add new content to "{{ $course->title }}"</p>
                </div>
                <a href="{{ route('instructor.courses.contents.index', $course) }}"
        ...
        <div class="max-w-4xl mx-auto">
            <form method="POST" action="{{ route('instructor.courses.contents.store', $course) }}" class="space-y-8"
                enctype="multipart/form-data"> {{-- ✅ Ini yang ditambahkan --}}
                @csrf
                            ...
                            <select id="topic_id" name="topic_id"
                                ...
                                @foreach ($topics as $topic)
                                    <option value="{{ $topic->id }}"
                                        {{ old('topic_id', request('topic')) == $topic->id ? 'selected' : '' }}>
                                        {{ $topic->title }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea id="material_content" name="material_content" rows="8"
                                ...
                                placeholder="Write your material content here...">{{ old('material_content') }}</textarea>
                        ...
                        <div>
                            ...
                            <input type="url" id="youtube_embed_url" name="youtube_embed_url"
                                value="{{ old('youtube_embed_url') }}"
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div>
                        ...
                        <textarea id="announcement" name="announcement" rows="4"
                            ...
                            placeholder="Any special announcements for this content...">{{ old('announcement') }}</textarea>
                        @error('announcement')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
