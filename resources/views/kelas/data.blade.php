<x-app-layout>
    <x-slot name="header">
        ...
        <button
            type="button"
            onclick="window.location='{{ url('kelas/create') }}'"
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            <tr class="{{ $number % 2 == 0 ? 'bg-white border-b dark:bg-gray-900' : 'bg-gray-100 dark:bg-gray-800' }}
                transition-colors duration-300 ease-in-out dark:hover:bg-blue-950 hover:text-black dark:hover:text-white">
                ...
                <td class="px-6 py-4">
                    <div class="flex items-center">
                        <button type="button" class="bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-2 rounded" onclick="window.location='{{ url('daftar-siswa') }}?id={{ $row->id }}'">Daftar Siswa</button><br>
                        <button type="button" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded ml-2" onclick="window.location='{{ route('kelas.edit',$row->id) }}'">Edit</button>
